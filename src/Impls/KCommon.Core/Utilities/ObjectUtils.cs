...
namespace KCommon.Core.Utilities
{
    ...
    public static class ObjectUtils
    {
        ...
        {
            var obj = new T();
            var propertiesFromSource = source.GetType()
            ...
            foreach (var property in properties)
            {
                ...
                if (sourceProperty != null) property.SetValue(obj, sourceProperty.GetValue(source, null), null);
            }
        ...
        public static void UpdateObject<TTarget, TSource>(TTarget target, TSource source,
            params Expression<Func<TSource, object>>[] propertyExpressionsFromSource)
            where TTarget : class
            where TSource : class
        {
            if (target == null) throw new ArgumentNullException("target");
            if (source == null) throw new ArgumentNullException("source");
            if (propertyExpressionsFromSource == null) throw new ArgumentNullException("propertyExpressionsFromSource");
        ...
        private static PropertyInfo GetProperty<TSource, TProperty>(Expression<Func<TSource, TProperty>> lambda)
        {
            ...
            if (type != propInfo.ReflectedType && !type.IsSubclassOf(propInfo.ReflectedType))
                throw new ArgumentException(string.Format(
                    "Expresion '{0}' refers to a property that is not from type {1}.", lambda.ToString(), type));
